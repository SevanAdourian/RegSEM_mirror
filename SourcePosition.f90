...
do n_src = 0, Tdomain%n_source-1
 ...
 if (Tdomain%curve) then
    ...
    Tdomain%Ssource(n_src)%realcolat = 90.d0 - Tdomain%Ssource(n_src)%realcolat
    if (Tdomain%Ssource(n_src)%reallong<0.d0) Tdomain%Ssource(n_src)%reallong = 360.d0 + Tdomain%Ssource(n_src)%reallong
    ...
    R = R - Tdomain%Ssource(n_src)%depth
    ...
    xs = Tdomain%Ssource(n_src)%realcolat
    ys = Tdomain%Ssource(n_src)%reallong
    zs = Tdomain%Ssource(n_src)%depth
 endif
 ...
 if (rg==Tdomain%sSource(n_src)%proc) then
    ...
    if (Tdomain%sSubDomain(mat)%material_type == "P") then
        print *,"SOURCE ", n_src, " OUT OF THE DOMAIN"
    ...
    do i = 0,2
       xi_search(i) = Tdomain%sSubdomain(mat)%GLLcx(x-1) + (i+1)*dist_xi/4.d0
       eta_search(i) = Tdomain%sSubdomain(mat)%GLLcy(y-1) + (i+1)*dist_eta/4.d0
       zeta_search(i) = Tdomain%sSubdomain(mat)%GLLcz(z-1) + (i+1)*dist_zeta/4.d0
    enddo
    ...
    dicho : do while (dmin > epsil)
       ...
       do i = 0,2
          xi_search(i) = centre(0) + (i-1)*dist_xi/4.d0
          eta_search(i) = centre(1) + (i-1)*dist_eta/4.d0
          zeta_search(i) = centre(2) + (i-1)*dist_zeta/4.d0
       enddo
       num = num + 1
 ...
 call mpi_bcast(src_hors_domaine,1,mpi_logical,Tdomain%sSource(n_src)%proc,mpi_comm_world,code)
 if (src_hors_domaine==.true..and.Tdomain%sSource(n_src)%i_type_source/=5)   stop
