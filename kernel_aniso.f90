...
!!! This is because save_strain comes from internal_forces.f90 where the displacement from the prediction phase is used !!!
              ...
              !write(*,*) "kern A is", kern_A_loc(x,y,z)
              !write(*,*) "kern C is", kern_C_loc(x,y,z)
              !write(*,*) "kern F is", kern_F_loc(x,y,z)
              !write(*,*) "kern L is", kern_L_loc(x,y,z)
              !write(*,*) "kern N is", kern_N_loc(x,y,z)
              !write(*,*) "kern_aniso 5 is and dt is", Tdomain%specel(n)%Kern_aniso(5,x,y,z), dt
          enddo
         enddo
        enddo
