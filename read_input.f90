...
integer :: length, i, j, mat, ok, nf, ne, pi
...
do i = 0, Tdomain%n_source - 1
   ...
   if(Tdomain%ellipticity)Tdomain%sSource(i)%realcolat=atan(0.99329534d0*dtan(Tdomain%sSource(i)%realcolat*PI/180.0d0))/PI*180.
   read (11,*) Tdomain%Ssource(i)%i_type_source
   print *, Tdomain%Ssource(i)%i_type_source
   if (Tdomain%Ssource(i)%i_type_source == 1) then
      ...
      if (Tdomain%Ssource(i)%i_time_function == 3.or.Tdomain%Ssource(i)%i_time_function == 6) then
   ...
   elseif (Tdomain%Ssource(i)%i_type_source == 5) then
   print *, 'DOING TELSEISMIC EVENTS'
      do j = 1,6
         read (11,*)
      enddo
   else
...
if (Tdomain%save_trace) then
    ...
    do i = 0, Tdomain%n_receivers-1
        ...
        if(Tdomain%ellipticity)Tdomain%sReceiver(i)%realcolat=atan(0.99329534d0*dtan(Tdomain%sReceiver(i)%realcolat*PI/180.0d0))/PI*180.
    enddo
    close (14)
endif
