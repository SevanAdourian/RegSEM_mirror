...
integer, parameter :: ecart=10, epsil=1, etiquette=100
integer, dimension(mpi_status_size) :: statut
doubleprecision :: vp,vs,rho,Qmu, ri,rj,rk, dx, x,y,z, r,theta,phi, A,C,L,M,F, Gc,Gs,Hc,Hs,Bc,Bs,Ec,Es, &
                   lambda,mu, xa,ya,za, xi,eta, Jac_surf, det, dt, lat,long, &
...
doubleprecision, dimension (1:3) :: barycentre
...
!!!!sfrench
write (*,*)
write (*,*) 'Warning: using SEMum2 / UCB transition zone discons!'
write (*,*)
...
if (meshtype==1 .or. meshtype==2 .or. meshtype==3 .or. meshtype==6) then
   ...
   allocate (rad(0:1))
   rad(0)=3480000; rad(1)=6366000 ! la CMB + une interface a 5km de profondeur
!else if (meshtype==0) then
!   allocate (rad(0:1))
!   rad(0)=-1000; rad(1)=55000 ! le bottom + une interface a z = 55km
endif
    ...
    do j = 0,nglly-1
     do i = 0,ngllx-1
        ...
        if (Tdomain%curve) then
           ...
           ! Ajustage maillage / modele de vitesse au niveau des interfaces spheriques 
           if (meshtype==1) then
              nb_interfaces = 11
...
!              call get_region_param (barycentre/1000.d0,rho,Cij)
!              call get_homog_param(rho,Cij)
!              call get_elastic_param (x,y,z,rho,Cij)
              Cij(:,:) = 0.d0
              Cij(1,1) = A+Bc+Ec
              Cij(2,2) = A-Bc+Ec
              Cij(3,3) = C
...
!     deallocate (Tdomain%specel(n)%InvGrad)
     ...
     if (Tdomain%sSubDomain(mat)%Px) then
         ...
         dx = dabs(Tdomain%GlobCoord(0,idef) - dx) 
         if (Tdomain%sSubDomain(mat)%Left) then
            do i = 0,ngllx-1
               ri = 0.5 * (1.d0 + Tdomain%sSubDomain(mat)%GLLcx(ngllx-1-i)) * dble(ngllx-1)
            ...
            do i = 0,ngllx-1
               ri = 0.5 * (1.d0 + Tdomain%sSubDomain(mat)%GLLcx(i)) * dble(ngllx-1)
               vp = Rkmod(i,0,0) / Tdomain%specel(n)%Density(i,0,0)
               vp = dsqrt(vp)
               wx(i,0:nglly-1,0:ngllz-1) = pow(ri, vp, ngllx-1, dx, Tdomain%sSubdomain(mat)%Apow, &
                                               Tdomain%sSubdomain(mat)%npow)
     ...
     if (Tdomain%sSubDomain(mat)%Py) then
         ...
         dx = dabs(Tdomain%GlobCoord (1,idef) - dx) 
         if (Tdomain%sSubDomain(mat)%Forward) then
            do j = 0,nglly-1
               rj = 0.5 * (1.d0 + Tdomain%sSubDomain(mat)%GLLcy(nglly-1-j)) * dble(nglly-1)
            ...
            do j = 0,nglly-1
               rj = 0.5 * (1.d0 + Tdomain%sSubDomain(mat)%GLLcy(j)) * dble(nglly-1)
               vp = Rkmod(0,j,0) / Tdomain%specel(n)%Density(0,j,0)
               vp = dsqrt(vp)
               wy(0:ngllx-1,j,0:ngllz-1) = pow(rj, vp, nglly-1, dx, Tdomain%sSubdomain(mat)%Apow, &
                                               Tdomain%sSubdomain(mat)%npow)
     ...
     if (Tdomain%sSubDomain(mat)%Pz) then
         ...
         dx = dabs(Tdomain%GlobCoord(2,idef) - dx)
         if (Tdomain%sSubDomain(mat)%Down) then
            do k = 0,ngllz-1
               rk = 0.5 * (1.d0 + Tdomain%sSubdomain(mat)%GLLcz(ngllz-1-k)) * dble(ngllz-1)
            ...
            do k = 0,ngllz-1
               rk = 0.5 * (1.d0 + Tdomain%sSubdomain(mat)%GLLcz(k)) * dble(ngllz-1)
               vp = Rkmod(0,0,k) / Tdomain%specel(n)%Density(0,0,k)
               vp = dsqrt(vp)
               wz(0:ngllx-1,0:nglly-1,k) = pow(rk, vp, ngllz-1, dx, Tdomain%sSubdomain(mat)%Apow, &
                                               Tdomain%sSubdomain(mat)%npow)
     ...
     if (Tdomain%FPML) then

         Tdomain%specel(n)%DumpSx(:,:,:,1) = Id + 0.5 * dt * wx * Tdomain%sSubdomain(mat)%freq
         Tdomain%specel(n)%DumpSx(:,:,:,1) = 1./ Tdomain%specel(n)%DumpSx (:,:,:,1)
         Tdomain%specel(n)%DumpSx(:,:,:,0) = (Id - dt * 0.5 * wx * Tdomain%sSubdomain(mat)%freq) *  Tdomain%specel(n)%DumpSx(:,:,:,1)

         Tdomain%specel(n)%DumpSy(:,:,:,1) = Id + 0.5 * dt * wy * Tdomain%sSubdomain(mat)%freq
         Tdomain%specel(n)%DumpSy(:,:,:,1) = 1./ Tdomain%specel(n)%DumpSy (:,:,:,1)
         Tdomain%specel(n)%DumpSy(:,:,:,0) = (Id - dt * 0.5 * wy * Tdomain%sSubdomain(mat)%freq) *  Tdomain%specel(n)%DumpSy(:,:,:,1)

         Tdomain%specel(n)%DumpSz(:,:,:,1) = Id + 0.5 * dt * wz * Tdomain%sSubdomain(mat)%freq
         Tdomain%specel(n)%DumpSz(:,:,:,1) = 1./ Tdomain%specel(n)%DumpSz (:,:,:,1)
         Tdomain%specel(n)%DumpSz(:,:,:,0) = (Id - dt * 0.5 * wz * Tdomain%sSubdomain(mat)%freq) *  Tdomain%specel(n)%DumpSz(:,:,:,1)

         Tdomain%specel(n)%DumpMass(:,:,:,0) =  Tdomain%specel(n)%Density * Whei * Jac * wx * &
                                                dt * 0.5 * Tdomain%sSubdomain(mat)%freq
         ...
         Tdomain%specel(n)%Isx = dt * wx * Tdomain%sSubdomain(mat)%freq * Tdomain%specel(n)%DumpSx(:,:,:,1)
         Tdomain%specel(n)%Isy = dt * wy * Tdomain%sSubdomain(mat)%freq * Tdomain%specel(n)%DumpSy(:,:,:,1)
         Tdomain%specel(n)%Isz = dt * wz * Tdomain%sSubdomain(mat)%freq * Tdomain%specel(n)%DumpSz(:,:,:,1)
                     
         Tdomain%specel(n)%Ivx = Tdomain%specel(n)%Density * Whei * dt * wx * Jac * Tdomain%sSubdomain(mat)%freq
         Tdomain%specel(n)%Ivy = Tdomain%specel(n)%Density * Whei * dt * wy * Jac * Tdomain%sSubdomain(mat)%freq
         Tdomain%specel(n)%Ivz = Tdomain%specel(n)%Density * Whei * dt * wz * Jac * Tdomain%sSubdomain(mat)%freq

     else

         Tdomain%specel(n)%DumpSx(:,:,:,1) = Id + 0.5 * dt * wx
         Tdomain%specel(n)%DumpSx(:,:,:,1) = 1./ Tdomain%specel(n)%DumpSx (:,:,:,1) 
         Tdomain%specel(n)%DumpSx (:,:,:,0) = (Id - dt * 0.5 * wx) * Tdomain%specel(n)%DumpSx(:,:,:,1)

         Tdomain%specel(n)%DumpSy(:,:,:,1) = Id + 0.5 * dt * wy
         Tdomain%specel(n)%DumpSy (:,:,:,1) = 1./ Tdomain%specel(n)%DumpSy (:,:,:,1) 
         Tdomain%specel(n)%DumpSy (:,:,:,0) = (Id - dt * 0.5 * wy) * Tdomain%specel(n)%DumpSy(:,:,:,1)

         Tdomain%specel(n)%DumpSz(:,:,:,1) = Id + 0.5 * dt * wz
         Tdomain%specel(n)%DumpSz(:,:,:,1)  = 1./ Tdomain%specel(n)%DumpSz (:,:,:,1)
         Tdomain%specel(n)%DumpSz (:,:,:,0) = (Id - dt * 0.5 * wz) * Tdomain%specel(n)%DumpSz(:,:,:,1)

         Tdomain%specel(n)%DumpMass(:,:,:,0) = 0.5 * Tdomain%specel(n)%Density * Whei * dt * wx * Jac
         Tdomain%specel(n)%DumpMass(:,:,:,1) = 0.5 * Tdomain%specel(n)%Density * Whei * dt * wy * Jac
         Tdomain%specel(n)%DumpMass(:,:,:,2) = 0.5 * Tdomain%specel(n)%Density * Whei * dt * wz * Jac
