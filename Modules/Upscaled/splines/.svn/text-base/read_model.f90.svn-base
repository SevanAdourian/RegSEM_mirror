...
! We could speed up the following by storing klo and khi in routine "splint" (cf p110 in F77 NR)
do k = 1,nb_pt_dir(3)
   do j = 1,nb_pt_dir(2)
      do i_count = 1,21
         y_C_val(j,i_count) = splint(xseries,C_homog(:,j,k,i_count),C_spline(:,j,k,i_count),x)
      enddo
      y_rho_val(j) = splint(xseries,rho_homog(:,j,k),rho_spline(:,j,k),x)
   enddo
   do i_count = 1,21
      call spline (yseries,y_C_val(:,i_count),1.d30,1.d30,y_spline)
      z_C_val(k,i_count) = splint(yseries,y_C_val(:,i_count),y_spline,y)
   enddo
   call spline (yseries,y_rho_val,1.d30,1.d30,y_spline)
   z_rho_val(k) = splint(yseries,y_rho_val,y_spline,y)
enddo
do i_count = 1,21
   call spline (zseries,z_C_val(:,i_count),1.d30,1.d30,z_spline)
   tmp_C(i_count) = splint(zseries,z_C_val(:,i_count),z_spline,z)
enddo
call spline (zseries,z_rho_val,1.d30,1.d30,z_spline)
rho = splint(zseries,z_rho_val,z_spline,z)
