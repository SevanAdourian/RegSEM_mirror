...
!==========================================
  ...
  integer :: nrec, n, i, ngll1, ngll2, ngll3, nsimu, nxint, nyint, nzint, mat
  ...
  character*100 :: snapfile, adjfile, coordfile
...
!       &.or.(Tdomain%t_reversal_mirror==2.and.mod(Tdomain%sTimeParam%ntime-ntime-2,Tdomain%sTimeParam%dnsnap)==0)))then
  ...
  if (Tdomain%save_snapshot .and. &
     ...
     do n = 0,Tdomain%n_elem-1
	   ...
	   !Displ(:,:,:,0,isimu)=Tdomain%specel(n)%mirror_position
	   !Displ(:,:,:,2,isimu)=0
           ...
           do zint = 0,nzint-1
              ...
              do yint = 0,nyint-1
                 ...
                 do xint = 0,nxint-1
                    ...
                    ! update : rotate coordinates to true global cartesian 
                    !(better for plotting multiple chunks) 
                    xa = xloc
                    ya = yloc
                    za = zloc
                    xloc = Tdomain%rot(0,0)*xa + Tdomain%rot(0,1)*ya + Tdomain%rot(0,2)*za       
                    yloc = Tdomain%rot(1,0)*xa + Tdomain%rot(1,1)*ya + Tdomain%rot(1,2)*za
                    zloc = Tdomain%rot(2,0)*xa + Tdomain%rot(2,1)*ya + Tdomain%rot(2,2)*za


                    ! Rotate displ vector as well for consistency
                             
                    do isimu = 0,nsimu
                      xa = displ_gll(isimu,0)   
                      ya = displ_gll(isimu,1)   
                      za = displ_gll(isimu,2)   
                      displ_gll(isimu,0) = Tdomain%rot(0,0)*xa + Tdomain%rot(0,1)*ya + Tdomain%rot(0,2)*za       
                      displ_gll(isimu,1) = Tdomain%rot(1,0)*xa + Tdomain%rot(1,1)*ya + Tdomain%rot(1,2)*za
                      displ_gll(isimu,2) = Tdomain%rot(2,0)*xa + Tdomain%rot(2,1)*ya + Tdomain%rot(2,2)*za
                    enddo
