...
integer :: n, i, ii, i1, i2, j1, j2, k1, k2, x, y, z, x1, x2, y1, y2, z1, z2, ngll_mirror
integer :: indx, indy, indz, ngllx, nglly, ngllz, rg, code
integer :: xs, ys, zs, nxtot, nytot, nztot, iglob, ngll_mirror_tot, count_gll
integer, dimension(:), allocatable :: glob_num_mirror, glob_num_mirror_all, sizes, displs, index_num
...
Tdomain%mirror_displ%spln_order = 5 ! B-Spline order
Tdomain%mirror_force%spln_order = 5 !
!
! Mirror geometry...
!
...
do n = 0,Tdomain%n_elem-1
   ...
   if(Tdomain%specel(n)%mirror_position == 1)then
      ...
      do z = 0,ngllz-1
         do y = 0,nglly-1
            do x = 0,ngllx-1
               if(Tdomain%specel(n)%win_mirror(x,y,z)/=0) i = i+1
            enddo
         enddo
      enddo
...
enddo
!
Tdomain%mirror_displ%recl_mirror = 3*i
Tdomain%mirror_force%recl_mirror = 3*i
!
