 ...
 do n = 0,Tdomain%n_elem-1
     ...
     if (.not. Tdomain%specel(n)%PML) then
        call internal_forces (Tdomain%specel(n), i_simu, Tdomain%sSubDomain(mat)%hprimex, &
                              ...
                              Tdomain%adjoint, Tdomain%t_reversal_mirror,.false.)
...
do n = 0, Tdomain%n_source-1
   if (rg == Tdomain%sSource(n)%proc) then
    if (Tdomain%sSource(n)%i_type_source==1 .or. Tdomain%sSource(n)%i_type_source==2) then
       ...
       if (Tdomain%sSource(n)%i_time_function == 3 .or. Tdomain%sSource(n)%i_time_function == 6 .or. Tdomain%sSource(n)%i_time_function == 5) then
   ...
   do n = 0, Tdomain%n_receivers-1
      if (rg == Tdomain%sReceiver(n)%proc) then
         ...
         if (Tdomain%sTimeParam%samp_period<dt .or. ntime==0) then
            if(mod(ntime+1,nt_between_write)==0.or.ntime==Tdomain%sTimeParam%ntime-1)then
             do iw = int(ntime/nt_between_write)*nt_between_write,ntime
              write (61+n*3,*) iw*dt, Tdomain%sReceiver(n)%StoreTrace(iw,0)
              write (62+n*3,*) iw*dt, Tdomain%sReceiver(n)%StoreTrace(iw,1)
              write (63+n*3,*) iw*dt, Tdomain%sReceiver(n)%StoreTrace(iw,2)
             enddo
            endif
         else    
